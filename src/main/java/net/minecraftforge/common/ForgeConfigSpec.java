...
public class ForgeConfigSpec extends UnmodifiableConfigWrapper<UnmodifiableConfig> //TODO: Remove extends and pipe everything through getSpec/getValues?
{
    ...
    public void setConfig(CommentedConfig config) {
        ...
        if (config != null && !isCorrect(config)) {
            ...
            LogManager.getLogger().warn("[Forge Core] Configuration file {} is not correct. Correcting", configName);
            correct(config,
                (action, path, incorrectValue, correctedValue) ->
                    LogManager.getLogger().warn("[Forge Core] Incorrect key {} was corrected from {} to its default, {}. {}", DOT_JOINER.join( path ), incorrectValue, correctedValue, incorrectValue == correctedValue ? "This seems to be an error." : ""),
                (action, path, incorrectValue, correctedValue) ->
                    LogManager.getLogger().debug("[Forge Core] The comment on key {} does not match the spec. This may create a backup.", DOT_JOINER.join( path )));
            ...
            return define(path, new ValueSpec(defaultSupplier, x -> x instanceof List && ((List<?>) x).stream().allMatch( elementValidator ), context) {
                ...
                public Object correct(Object value) {
                    if (value == null || !(value instanceof List) || ((List<?>)value).isEmpty()) {
                        LogManager.getLogger().debug("[Forge Core] List on key {} is deemed to need correction. It is null, not a list, or an empty list. Modders, consider defineListAllowEmpty?", path.get(path.size() - 1));
                        return getDefault();
                    }
                    List<?> list = Lists.newArrayList((List<?>) value);
                    list.removeIf(elementValidator.negate());
                    if (list.isEmpty()) {
                        LogManager.getLogger().debug("[Forge Core] List on key {} is deemed to need correction. It failed validation.", path.get(path.size() - 1));
                        return getDefault();
                    }
                    return list;
                }
            ...
            return define(path, new ValueSpec(defaultSupplier, x -> x instanceof List && ((List<?>) x).stream().allMatch( elementValidator ), context) {
                ...
                public Object correct(Object value) {
                    if (value == null || !(value instanceof List)) {
                        LogManager.getLogger().debug("[Forge Core] List on key {} is deemed to need correction, as it is null or not a list.", path.get(path.size() - 1));
                        return getDefault();
                    }
                    List<?> list = Lists.newArrayList((List<?>) value);
                    list.removeIf(elementValidator.negate());
    ...
    private static class Range<V extends Comparable<? super V>> implements Predicate<Object>
    {
        ...
        public boolean test(Object t)
        {
            if (isNumber(t))
            {
                ...
                if(!result)
                {
                    LogManager.getLogger().debug("[Forge Core] Range value {} is not within its bounds {}-{}", n.doubleValue(), ((Number)min).doubleValue(), ((Number)max).doubleValue());
                }
                return result;
            }
            ...
            if(!result)
            {
                LogManager.getLogger().debug("[Forge Core] Range value {} is not within its bounds {}-{}", c, min, max);
            }
            return result;
        }
